...
const getPortalDomains = async (accountId) => {
  try {
    ...
    logger.error("There was a problem fetching domains for your portal. You may be missing a scope necessary for this feature.")
    return [];
  }
}
...
const trackPreviewEvent = async (action) => {
  ...
  trackUsage(
    ...
    (err) => {
      logger.error(`trackUsage failed: ${JSON.stringify(err, null, 2)}`);
    }
  );
}
