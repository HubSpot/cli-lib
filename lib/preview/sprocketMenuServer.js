...
const startSprocketMenuServer = async (sessionInfo) => {
  const portIsTaken = await new Promise((res, rej) => {
    logger.log(`Starting proxy link server on port ${SPROCKET_MENU_PORT}`);
    const testNetServer = net.createServer();

    testNetServer.once('error', err => {
      if (err['code'] === 'EADDRINUSE') {
        logger.error(
          `Port ${SPROCKET_MENU_PORT} is in use. HubSpot is unable to automatically create proxy links in the Sprocket Menu`
        );
        res(true);
      } else {
        rej(err);
