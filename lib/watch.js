...
const { isConvertibleFieldJsFile, FieldsJs } = require('./handleFieldsJs');
...
async function uploadFile(accountId, file, dest, options) {
  ...
  const convertFields = isConvertibleFieldJsFile(
    src,
    file,
...
function watch(
  ...
  if (remove) {
    const deleteFileOrFolder = type => filePath => {
      ...
      if (
        isConvertibleFieldJsFile(src, filePath, commandOptions.convertFields)
      ) {
        return;
      }
