...
async function uploadFolder(
  ...
  const onAttemptCallback_ = onAttemptCallback || ((file, destPath) => logger.debug(
    'Attempting to upload file "%s" to "%s"',
    file,
    destPath
  ));
  const onSuccessCallback_ = onSuccessCallback || ((file, destPath) => logger.log(
    'Uploaded file "%s" to "%s"',
    file,
    destPath
  ));
  const onFirstErrorCallback_ = onFirstErrorCallback || ((file, destPath, error) => {
    logger.debug(
  ...
  const onRetryCallback_ = onRetryCallback || ((file, destPath) => logger.debug(
    'Retrying to upload file "%s" to "%s"',
    file,
    destPath
  ))
  const onFinalErrorCallback_ = onFinalErrorCallback || ((file, destPath, error) => {
  ...
  const uploadFile = file => {
    ...
    return async () => {
      onAttemptCallback_(originalFilePath, destPath);
      try {
        await upload(accountId, file, destPath, apiOptions);
        onSuccessCallback_(originalFilePath, destPath);
      } catch (error) {
        if (isFatalError(error)) {
          throw error;
        }
        onFirstErrorCallback_(file, destPath);
        failures.push({
          file,
          destPath,
        });
  ...
  const results = await queue
    .addAll(
      failures.map(({ file, destPath }) => {
        return async () => {
          onRetryCallback_(file, destPath);
          try {
            await upload(accountId, file, destPath, apiOptions);
            onSuccessCallback_(file, destPath);
            return {
            ...
            if (isFatalError(error)) {
              ...
              onFinalErrorCallback_(file, destPath, error);
            return {
