...
describe('handleFieldsJs', () => {
  ...
  describe('isConvertibleFieldJsFile()', () => {
    const src = 'folder';

    it('returns true for root fields.js files', () => {
      const filePath = 'folder/fields.js';
      const returned = isConvertibleFieldJsFile(src, filePath, true);
    ...
    it('returns false for any other file name', () => {
      expect(isConvertibleFieldJsFile(src, 'folder/fields.json')).toBe(false);
      expect(
        isConvertibleFieldJsFile(src, 'folder/sample.module/fields.json', true)
      ).toBe(false);
      expect(isConvertibleFieldJsFile(src, 'folder/js/example.js', true)).toBe(
        false
      );
    });
  });
