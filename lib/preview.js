...
const { startShadowDevService } = require('./preview/shadowDevServer')
const { startHttpsRedirectingService } = require('./preview/httpsRedirectingServer');
...
const preview = async (
  ...
  src,
  dest,
  { notify, filePaths, initialUpload, noSsl }
) => {
  ...
  const expressServer = await startLocalHttpService(sessionInfo);

  if (!noSsl) {
    const { server, innerHTTPServer, innerHTTPSServer } = await startHttpsRedirectingService(expressServer, domains);
    server.listen(3000);
    console.log('Now listening on port 3000 with HTTPS')
  } else {
    const httpServer = http.createServer(expressServer);

    httpServer.listen(3000);
    console.log('Now listening on port 3000 with HTTP');
  }
  startShadowDevService(sessionInfo);
}
