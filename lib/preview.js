...
const { uploadFolder, FileUploadResultType } = require('./uploadFolder');
...
const initialPreviewBufferUpload = async (sessionInfo, filePaths) => {
  ...
  progressBar.start(filePaths.length, 0, {
    label: i18n(`${i18nKey}.initialProgressBar.start`),
  });
    ...
    onSuccessCallback: () => {
      ...
      if (!uploadsHaveStarted) {
        ...
        progressBar.update(0, {
          label: i18n(`${i18nKey}.initialProgressBar.uploading`)
        })
      }
    },
    onFirstErrorCallback: () => { /* Intentionally blank */ },
    onRetryCallback: () => { /* Intentionally blank */},
    onFinalErrorCallback: () => progressBar.increment()
  };
  const results = await uploadFolder(accountId, src, dest, fileMapperArgs, uploadOptions, filePaths);
  progressBar.update(filePaths.length, {
    label: i18n(`${i18nKey}.initialProgressBar.finish`)
  });
  progressBar.stop();
